        ...
        if(data.verifyotp==req.body.verifyotp){
            userquery.findByIdAndUpdate(req.body.queryid,{"custverifystatus":true,"status":"completed"},function(err){
                if(err){
                    ...
                    /// have to depoly coins here
                    let x=0;
                    usersdata.findById(req.body.userid,function(err,usercoin){
                        console.log('previous coins',usercoin.coins);

                        usercoin.coins=usercoin.coins+0.5;
                        usersdata.findByIdAndUpdate(req.body.userid,{"coins":usercoin.coins},function(err){
                            if(err){
                                console.log('errror in coins: ',err);
                            }
                            else{
                                console.log('updated coins',usercoin.coins);
                            }
                        });
